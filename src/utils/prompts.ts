...
const codeQuery = (type: ValueSetSystem, primary?: boolean) =>
  z.object({
...
const codeStructure = (
  ...
  isRes ? z.union([code, codeQuery(type, primary)]) : codeQuery(type, primary);
        ...
        hospitalization: z
          .object({
            ...
            admit_source: z
              ...
              .describe(
                `Admission source out of : "hosp_trans" (Hospital Transfer) | "emd" (Emergency Department) | "outp" (Outpatient Department) | "born" (Born) | "gp" (General Practitioner) | "mp" (Medical Practitioner) | "nursing" (Nursing Home) | "psych" (Psychiatric Hospital) | "rehab" (Rehabilitation Facility) | "other" (Other)`,
              ),
            diet_preference: z
              .enum([
                "vegetarian",
            ...
            discharge_disposition: z
              ...
              .optional()
              .describe(
                `Only applicable if status is "completed". Choose from "home" (Home) | "alt_home" (Alternate Home) | "other_hcf" (Other Healthcare Facility) | "hosp" (Hospice) | "long" (Long Term Care) | "aadvice" (Left Against Advice) | "exp" (Expired) | "psy" (Psychiatric Hospital) | "rehab" (Rehabilitation) | "snf" (Skilled Nursing Facility) | "oth" (Other)`,
              ),
          })
          .optional()
          .describe(
      ...
      z.array(
        z.object({
          ...
          category: withFallback(
            z.enum(["inpatient", "outpatient", "community", "discharge"]),
            "inpatient",
          ),
          priority: withFallback(
          ...
          dosage_instruction: z.array(
            z.object({
              sequence: z.number().optional(),
              text: z.string().optional(),
              additional_instruction: z
                .array(codeStructure(isRes, "system-additional-instruction"))
                .optional(),
                  ...
                  repeat: z
                    .object({
                      ...
                      period: z
                        .number()
                        .describe(
                          "number of units (ex. 12 days would mean 12 with unit 'd')",
                        ),
                      period_unit: z.enum([
                        "s",
                        "min",
                  ...
                  code: z.object({
                    code: z.enum(
                      Object.values(MEDICATION_REQUEST_TIMING_OPTIONS).map(
                        (timing) => timing.timing.code.code,
                      ) as [string],
                    ),
                    display: z.enum(
                      Object.values(MEDICATION_REQUEST_TIMING_OPTIONS).map(
              ...
              as_needed_boolean: withFallback(
                z
                  .boolean()
                  .describe(
                    "True if the prescription is PRN, else false. Do not ommit this.",
                  ),
                false,
              ),
              as_needed_for: codeStructure(isRes, "system-as-needed-reason")
                .optional()
                .describe(
                  "If it is a PRN medication (as_needed_boolean is true), the indicator",
                ),
              site: codeStructure(isRes, "system-body-site")
                .optional()
                .describe("The site the medication should be administered at"),
              route: codeStructure(isRes, "system-route").optional(),
              method: codeStructure(
                isRes,
    ...
    prompt: (isRes?: boolean) =>
      z.array(
        z.object({
          ...
          onset: withFallback(
            z.object({ onset_datetime: isoDateTime }).describe("ISO format"),
    ...
    prompt: (isRes?: boolean) =>
      z.array(
        z.object({
          ...
          recorded_date: withFallback(
            isoDateTime.describe("In ISO format"),
            new Date().toISOString(),
          ),
          note: z.string().optional(),
          category: withFallback(
            z
              .enum(["encounter_diagnosis", "chronic_condition"])
              .describe(
                `Is this a chronic condition or a normal encounter diagnosis?`,
              ),
            "encounter_diagnosis",
          ),
        }),
