...
const toolStructure = z.object({
  ...
  external_identifier: z.string().nullable().describe("ip/op/obs/emr number"),
  readmission: z
    .boolean()
    .describe(
...
export const encounterStructure: Structure<Encounter[], typeof toolStructure> =
  {
    ...
    toolStructure,
    deserialize: async (data, currentDataArray) => {
      const currentData = currentDataArray?.[0] || {};
      return {
        data: [
          {
            ...currentData,
            status: data.encounter_status || currentData?.status,
            encounter_class:
              data.encounter_class || currentData?.encounter_class,
            priority: data.encounter_priority || currentData?.priority,
            external_identifier:
              data.external_identifier || currentData?.external_identifier,
            hospitalization: ["imp", "absenc", "emer"].includes(
              ...
              ? {
                  re_admission:
                    data.readmission ||
                    currentData?.hospitalization?.re_admission ||
                    false,
                  admit_source:
                    data.admit_source ||
                    currentData?.hospitalization?.admit_source,
                  diet_preference:
                    data.diet_preference ||
                    currentData?.hospitalization?.diet_preference,
                  discharge_disposition:
                    data.discharge_disposition ||
                    currentData?.hospitalization?.discharge_disposition,
                }
              : undefined,
          },
        ],
